...
##' \code{\link{linear.scale}} but users can write their own function
##' for this purpose. User-defined scaling functions need to be of the
##' same form as \code{\link{linear.scale}}, i.e. it must have
##' arguments for the input vector \code{x}, and the \code{min} and
##' \code{max}.
##' 
##' @param time The desired start time mapping of all events (in
##' seconds). A list with three elements: the minimum time, maximum
##' time, and the scaling function.
##' @param pitch The desired pitch mapping of all events, represented
...
##' @param dur The desired duration mapping (in seconds). A list with
##' three elements: the minimum duration, the maximum duration, and
##' the scaling function.
##' @param vol The desired volume mapping as a number between 0,
##' silence, and 1, the maximum possible amplitude. A list with three
##' elements: the minimum volume, the maximum volume, and the scaling
##' function.
##' @param pan The desired mapping for balance between left and right
##' stereo channels, where 0 is all the left channel, 0.5 balanced,
##' and 1 all the right. A list with three elements: the minimum pan,
##' the maximum pan, and the scaling function.
##' @param tempo The desired tempo mapping (in beats per minute). A
##' list with three elements: the minimum tempo, the maximum tempo,
