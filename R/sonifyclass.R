...
##' 
##' Methods to print and alter sonify objects. Printing a sonify object renders
##' it to sound (analogously to how printing \code{ggplot} objects renders them
##' to screen in the \href{http://had.co.nz/ggplot2}{ggplot2} package).
##' 
##' The \code{print} method for \code{sonify} objects not only renders them to
##' sound, it also creates an \code{audioSample} object (from the \code{audio}
##' package), \code{.LastRendering}, in the user's workspace.
##' \code{.LastRendering} is the output of the sonification and can be replayed
##' or saved to a \code{.wav} file without regenerating the sonification from
##' scratch with the convenience functions \code{\link{playLastRendering}} and
...
##' @param y  A \code{sonscaling} object generated by
##' \code{scaling()} or a \code{sonaes} object generated by \code{sonaes()} to
##' add to \code{x}.
##' 
##' @param \dots Ignored.
##' @return \code{+} and \code{\%+\%} both return a sonify object with the
##' relevant \code{y} value added into the object.
##' 
##' \code{print.sonify} is called for its side-effect, which is to actually
##' render the object to a sound.
##' 
##' \code{summary.sonify} prints a brief summary of the name of the dataset to
##' be sonified, and the specified mappings and scalings of sonic values to
##' data parameters.
##' @seealso \code{\link{sonify}} for the creation of these objects; \code{\link{\%+\%}} for adding or replacing a default \code{data.frame}.
##' @method print sonify
##' @export
